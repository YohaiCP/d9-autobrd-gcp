...
exports.listProjects = async () => {
  var r = await google.cloudresourcemanager('v1').projects.list({ pageSize: GCP_PROJECT_LIST_LIMIT, filter: 'lifecycleState:active AND NOT labels.dome9ignore:true AND NOT labels.dome9onboarded:true' });
  return r.data['projects'] ? r.data['projects'] : [];
};

exports.addOnboardedLabel = async (project) => {
  var result = true;
  var labels = project['labels'] ? project['labels'] : {};
  labels.dome9onboarded = 'true';
  var req = {
